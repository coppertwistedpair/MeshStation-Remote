...
KEEP_FILES=(
  ...
  "lib/python3.10/lib-dynload/_random.cpython-310-aarch64-linux-gnu.so"
  "lib/python3.10/lib-dynload/_opcode.cpython-310-aarch64-linux-gnu.so"
  "lib/python3.10/lib-dynload/binascii.cpython-310-aarch64-linux-gnu.so"
  "lib/python3.10/lib-dynload/_hashlib.cpython-310-aarch64-linux-gnu.so"
  "lib/python3.10/lib-dynload/_ssl.cpython-310-aarch64-linux-gnu.so"
  "lib/python3.10/lib-dynload/_bisect.cpython-310-aarch64-linux-gnu.so"
...
build_env() {
  ...
  "$MM" create -p "$ENV_PATH" -f "./linuxenv.yml" --yes
}

test_env() {
  echo "Testing the environment..."
  "$MM" run -p "$ENV_PATH" python -c "import gnuradio, pmt, osmosdr, numpy, zmq; import gnuradio.lora_sdr as l; print('OK')"
}
...
prune_python_sos() {
  ...
  if [[ -d "$d2" ]]; then
    while IFS= read -r f; do
      # Keep the whole pyzmq package (ZMQ PUB output)
      [[ "$f" == */site-packages/zmq/* ]] && continue
